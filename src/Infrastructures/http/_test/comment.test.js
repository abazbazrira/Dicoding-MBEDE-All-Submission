  ...
  describe('when POST /threads/{threadId}/comments', () => {
    it('should response 201 and presisted comments', async () => {
      ...
      const postCommentResponse = await server.inject({
        method: 'POST',
        url: `/threads/${addedNewThread.id}/comments`,
        payload: {
      ...
      const result = await CommentsTableTestHelper.findCommentById(addedNewComment.id);
  ...
  describe('when DELETE /threads/{threadId}/comments/{commentId}', () => {
    it('should response 200 and delete comment', async () => {
      ...
      const deleteCommentResponse = await server.inject({
        method: 'DELETE',
        url: `/threads/${addedNewThread.id}/comments/${addedNewComment.id}`,
        headers: { Authorization: `Bearer ${accessToken}` },
      });
